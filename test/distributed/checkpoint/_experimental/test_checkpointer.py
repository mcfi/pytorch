...
class TestCheckpointer(TestCase):
    ...
    def _save_checkpoint(self, checkpointer: Checkpointer, state_dict, path, **kwargs):
        """Save checkpoint and handle both sync/async return values."""
        result = checkpointer.save(state_dict, path, **kwargs)
            ...
            with self.subTest(checkpointer_type=checkpointer_type):
                try:
                    ...
                    stage_future, write_future = self._save_checkpoint(
                        checkpointer, self.state_dict, checkpoint_path
                    )
        ...
        for checkpointer_type, checkpointer in self._get_checkpointers():
            with self.subTest(checkpointer_type=checkpointer_type):
                try:
                    ...
                    stage_future, write_future = self._save_checkpoint(
                        checkpointer, limited_state_dict, checkpoint_path
                    )
                    self._wait_for_save(stage_future, write_future)
        ...
        for checkpointer_type, checkpointer in self._get_checkpointers():
            with self.subTest(checkpointer_type=checkpointer_type):
                try:
                    ...
                    stage_future, write_future = self._save_checkpoint(
                        checkpointer, nested_state_dict, checkpoint_path
                    )
                    self._wait_for_save(stage_future, write_future)
...
class TestAsyncCheckpointer(TestCase):
    ...
    def test_async_sequential_saves_wait(self):
        ...
        try:
            ...
            stage_future1, write_future1 = checkpointer.save(
                self.state_dict, checkpoint_path1
            )
            ...
            stage_future2, write_future2 = checkpointer.save(
                modified_state_dict, checkpoint_path2
            )
    ...
    def test_async_error_handling(self):
        ...
        try:
            ...
            stage_future, write_future = checkpointer.save(self.state_dict, "/tmp/test")
