...
def _create_tensor_from_bytes(
    tensor_bytes: bytes,
    tensor_meta: schema.TensorMeta,
) -> torch.Tensor:
    """Create a tensor from raw bytes and tensor metadata."""
    ...
    if len(tensor_bytes) != 0:
        ...
        tensor = torch.zeros(size, dtype=dtype).to(device)
...
def _build_file_map(
    ...
    """Build a map from file path to the saved payload."""
    ...
    for payload_meta in config.config.values():
        ...
        tensor = _create_tensor_from_bytes(tensor_bytes, payload_meta.tensor_meta)
...
def _load_state_dict(
    ...
    if legacy_weights_file in archive_reader.get_file_names():
        ...
        weights_config = _dict_to_dataclass(
            schema.PayloadConfig,
            json.loads(
                archive_reader.read_string(WEIGHTS_CONFIG_FORMAT.format(model_name))
            ),
        )
        # construct the mapping from file name (e.g. weight_0) to weight payload
        state_dict_file_map = _build_file_map(
            archive_reader, weights_config, WEIGHTS_DIR
        )
        # chain the mapping weight FQN -> weight file name -> weight payload
...
def _load_constants(
    ...
    if legacy_constants_file in archive_reader.get_file_names():
        ...
        constants_config = _dict_to_dataclass(
            schema.PayloadConfig,
            json.loads(
                archive_reader.read_string(CONSTANTS_CONFIG_FORMAT.format(model_name))
            ),
        )
        ...
        # chain the mapping constant FQN -> constant file name -> constant payload
